...
wall_raw_xlsx = 'Input Sheets(B)_plate modified.xlsx'
...
def ErrorSection(value_min_list, value_max_list, max_criteria):
    ...
    for error_value_min, error_value_max, error_value_index in zip(value_min_list, value_max_list, value_min_list.index):
        if (error_value_min >= max_criteria):
            if (error_value_max > error_value_min):
...
DCR_output = pd.concat([SF_ongoing.iloc[:,[0,1,12]].dropna(), DCR_total.iloc[:,0:8].max(axis=1)], axis=1)

DCR_output['Result'] = np.where(DCR_output.iloc[:,3] >= DCR_criteria, 'NG', 'OK')

DCR_output.columns = ['부재명', '층', '수평 배근', '최대 DCR', '결과']
