...
# Wall input 정보 (xlsx)
wall_raw_xlsx_dir = r'D:\이형우\내진성능평가\광명 4R\103'
wall_raw_xlsx = 'Input Sheets(103).xlsx'
...
# 해석 output 파일 (txt)
H1_txt_dir = r'D:\이형우\내진성능평가\광명 4R\해석 결과\103\H1'
H2_txt_dir = r'D:\이형우\내진성능평가\광명 4R\해석 결과\103\H2'
V_txt_dir = r'D:\이형우\내진성능평가\광명 4R\해석 결과\103\V'
...
# V. Rebar 나누기
wall['Vertical Rebar(DXX)'] = wall['Vertical Rebar(DXX)'].astype(str)
wall['Horizontal Rebar(DXX)'] = wall['Horizontal Rebar(DXX)'].astype(str)

first = []
second = []

for i, j in zip(wall['Vertical Rebar(DXX)'], wall['V. Rebar Space']):
    if '-' in i:
        second.append(i.split('-')[0])
        first.append(i.split('-')[1])
    else:
...
wall['V. Rebar Space'] = second
wall['Vertical Rebar(DXX)'] = first

wall['V. Rebar Space'] = wall['V. Rebar Space'].astype(int)
...
def FindPosition_max(series_max, txt_name):  # Series형태로 입력
    time_history = pd.read_csv(txt_name, skiprows=len(series_max)+7, header=None, encoding='cp949', usecols=[i for i in range(1,len(series_max)+1)])[:-2]  # header 어케하는게 좋을까?    
...
def FindPosition_min(series_min, txt_name):  # Series형태로 입력
    time_history = pd.read_csv(txt_name, skiprows=len(series_min)+7, header=None, encoding='cp949', usecols=[i for i in range(1, len(series_min)+1)])[:-2]  # header 어케하는게 좋을까?    
...
def MatchV(series_max_or_min, max_or_min_position, v_txt_name):
    v_time_history = pd.read_csv(v_txt_name, skiprows=len(series_max_or_min)+7, header=None, encoding='cp949', usecols=[i for i in range(1, len(series_max_or_min)+1)])[:-2]
...
# Section name열 추출
section_name = pd.read_csv(H1_DE11_raw_txt, skiprows=7, error_bad_lines=False, header=None, encoding='cp949').iloc[:, -1]
...
DCR_total = pd.concat([DCR_H1_min.iloc[:, 0:14].mean(axis=1), DCR_H1_max.iloc[:, 0:14].mean(axis=1),\
                       DCR_H1_min.iloc[:, 14:28].mean(axis=1), DCR_H1_max.iloc[:, 14:28].mean(axis=1),\
                       DCR_H2_min.iloc[:, 0:14].mean(axis=1), DCR_H2_max.iloc[:, 0:14].mean(axis=1),\
                       DCR_H2_min.iloc[:, 14:28].mean(axis=1), DCR_H2_max.iloc[:, 14:28].mean(axis=1)],\
